...
def transfer_tags(source_bam, target_bam, output_bam):
    # Open the source BAM (with ML/MM tags) and target BAM
    source = pysam.AlignmentFile(source_bam, "rb")
    target = pysam.AlignmentFile(target_bam, "rb")
    output = pysam.AlignmentFile(output_bam, "wb", template=target)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Transfer ML/MM tags from one BAM file to another based on read IDs, and output only primary alignments with transferred tags.")
    parser.add_argument("-s", "--source", required=True, help="Source BAM file (with ML/MM tags)")
    parser.add_argument("-t", "--target", required=True, help="Target BAM file (without ML/MM tags)")
    parser.add_argument("-o", "--output", required=True, help="Output BAM file with transferred tags")
