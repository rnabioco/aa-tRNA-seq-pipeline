...
rule classify_charging_leech:
    ...
    GPU-accelerated alternative to remora (requires leech, installed by `pixi run setup`)

    Writes the ML (score) and MP (query position) tags. Note that leech writes MP
    where remora writes MM, so downstream tag transfer differs between classifiers.
...
rule classify_aa_identity:
    ...
    Adds aa (predicted AA), ac (confidence), am (margin), pn (pair names) and
    pp (pair probs) tags. --raw keeps ac/am/pp as floats; without it leech
    encodes them as uint8.
    """
    input:
        pod5=get_classification_pod5,
        bam=rules.inject_ubam_tags.output.bam,
