...
rule collect_escpod_pod5:
    ...
    escpod demux already wrote one POD5 per barcode, so this is a symlink rather
    than a second filter pass. The link and its target share the split_pod5 cleanup
    tier so they are removed together.
    """
    input:
    ...
    shell:
        ...
        if [ ! -f "{params.src}" ]; then
            ...
            echo "escpod demux emits one POD5 per class in its barcode model;" >&2
            echo "this sample's barcode is not among them. Check that the" >&2
            echo "samples file barcodes match the model in" >&2
