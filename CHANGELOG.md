...
### Changed
- **`escpod` repinned 0.17.1 -> 0.18.1**, all five release tarballs rechecksummed from the release's `SHA256SUMS.txt`. This is the first version `classify_charging` can run on at all. `signal classify` located the CCA|adapter junction by requiring `CCAGGC` to occur exactly once per reference record, and checked that *before* the common arm that actually identifies the junction — but the motif is only CCA plus the adapter's opening bases, so a tRNA body carries it by chance in ~1.5% of records (4 of 282 in hg38, 51 of 3315 in danRer11), and one such record aborted the entire run for that sample. Pruning the offenders is not neutral: 48 of danRer11's 51 are Glu. Reported as escapepod-rs#298, fixed in escapepod-rs#299. **Do not pin 0.18.0** — it was tagged at 02:17:02Z and the fix merged at 02:18:55Z, so it does not contain it.
- **`escapepod_demux` now asks for two GPUs.** 0.18.0's encoder pool uses every visible device instead of reserving one for adapter detection (~5% of device time), taking two GPUs from 1.19x to **2.01x** against one. Two further changes land on this rule's exact configuration: `--ref-scores`, which `ldx.ref_scores: true` enables, gained a GPU decode path (38.7 s -> 31.5 s, 5.9 cores -> 1.3 on 100k reads); and the encoder's onnxruntime sessions no longer spin, where each worker previously built an intra-op pool `--threads` wide — our `--threads 32` with two workers put 64 spinning threads on a 32-core allocation. Barcode calls are unaffected by all three (identical on 100,654 reads upstream; `crf_logp`, `crf_margin`, `mean_logpost` agree to 2e-4), so re-running demux under 0.18.1 is a speed change, not a result change.
- Verified before pinning, because 0.17.0 taught us not to trust a version number: the `v0.18.1` GitHub Release exists with all five binary artifacts **including the GPU build**; both x86_64 Linux tarballs were downloaded and hashed independently against `SHA256SUMS.txt`; the GPU binary contains `CUDAExecutionProvider` (absent from the musl build) and self-reports `escpod 0.18.1`; and `compare/88c5fb2b...v0.18.1` returns `ahead, behind_by=0`, confirming the tag actually contains the junction fix rather than merely postdating it.
